...
int get_block(int dev, int blk, char *buf)
{
   ...
   int blocksread = read(dev, buf, BLKSIZE);
}   
...
int idalloc(int dev, int ino)
{
   ...
   put_block(dev, mptr->imap, buf); // write buf back to imap.
}
